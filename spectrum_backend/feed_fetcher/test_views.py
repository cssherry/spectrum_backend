...
from .views import get_associated_articles, test_api
...
class GetAssociationsTestCase(TestCase):
    ...
    def test_api_returns_recent_feed_items(self):
        ...
        self.assertEquals(response.status_code, 200)
        self.assertIn(self.association.associated_feed_item.base_object(), json.loads(response.content.decode()))
        self.assertIn(self.association.base_feed_item.base_object(), json.loads(response.content.decode()))
