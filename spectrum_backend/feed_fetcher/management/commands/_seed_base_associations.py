...
class SeedBaseAssociations(BaseCommand):
    def seed(self):
        ...
        app.process_formation()['worker'].resize(2) # We scale this process up to ensure sufficient memory
        feed_items = FeedItem.recent_items_eligible_for_association(settings.DAYS_TO_CHECK_FOR)
        main(feed_items)
        app.process_formation()['worker'].resize(1) # We scale back down to avoid extra costs